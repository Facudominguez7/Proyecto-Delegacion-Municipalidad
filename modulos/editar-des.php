...
if (isset($_GET['accion'])) {
    if ($_GET['accion'] == 'editar') {
        ...
        $sql = "UPDATE desmalezamientos SET titulo=?, idChacra=?, ubicacion=POINT(?, ?) WHERE id=?";
        $stmt = mysqli_prepare($con, $sql);
        mysqli_stmt_bind_param($stmt, "sissi", $titulo, $idChacra, $latitud, $longitud, $idDes);
...
<section>
    <div class="flex items-center justify-center p-12">
        <div class="mx-auto w-full max-w-[550px]">
            <?php
            $idDes = $_GET['idDes'];
            $sql = "SELECT des.id, des.titulo, chacras.nombre as nombreChacra, chacras.id as idChacra, ST_X(des.ubicacion) as latitud, ST_Y(des.ubicacion) as longitud 
            ...
            ?>
            <form action="index.php?modulo=editar-des&accion=editar" method="POST">
                <div class="mb-5">
                    <label for="name" class="mb-3 block text-base font-medium text-[#07074D]">
                        Dirección
                    </label>
                    <input type="text" name="titulo" id="titulo" value="<?php echo $r['titulo'] ?>" placeholder="Dirección" class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                </div>
                <div class="mb-5">
