...
if (!empty($_GET['accion'])) {
    ...
    if ($_GET['accion'] == 'agregar') {
        ...
        $sql_verificar = "SELECT * FROM tareas_puntos WHERE titulo = ?";
        ...
        if (mysqli_stmt_num_rows($stmt_verificar) != 0) {
            echo "<script> alert('ESTA TAREA YA EXISTE EN LA BASE DE DATOS');</script>";
        } else {
            // Insertar datos en la tabla de tareas
            $sqlInsertTarea = "INSERT INTO tareas_puntos(titulo, fecha, descripcion, idPunto) VALUES (?, ?, ?, ?)";
            $stmtTarea = mysqli_prepare($con, $sqlInsertTarea);
            mysqli_stmt_bind_param($stmtTarea, "sssi", $titulo, $fecha, $descripcion, $idPunto);
            ...
            if (mysqli_error($con)) {
                ...
                $carpeta_carga = "imagenes/tareas_puntos/" . $tarea_id . "/";
                ...
                foreach ($_FILES["lista"]["tmp_name"] as $key => $tmp_name) {
                    ...
                    if (move_uploaded_file($tmp_name, $archivoDestino)) {
                        ...
                        $sqlInsertImagen = "INSERT INTO fotos_puntos (idTareaPunto, nombreArchivo) VALUES (?, ?)";
            ...
            echo "<script>window.location='index.php?modulo=tareas-puntos&idPunto=" . $idPunto . "';</script>";
        }
    }
}
?>
...
<section>
    <div class="flex items-center justify-center p-12">
        <div class="mx-auto w-full max-w-[550px]">
            <form action="index.php?modulo=agregar-tarea-punto&accion=agregar&idPunto=<?php echo $_GET['idPunto'] ?>" method="POST" enctype="multipart/form-data">
                <div class="mb-5">
