...
if (!empty($_GET['accion'])) {
    if ($_GET['accion'] == 'agregar') {
        ...
        $sqlInsertPuntos = "INSERT INTO tareas_apoyo(idChacra, titulo, ubicacion) VALUES (?, ?, ST_GeomFromText(?))";
        ...
        if (mysqli_error($con)) {
            ...
            echo "<script>alert('Tarea de apoyo registrada correctamente');</script>";

            echo "<script>window.location='index.php?modulo=tareas-apoyo';</script>";
        }
    }
}
?>
...
<section>
    ...
    <div class="flex items-center justify-center p-12">
        <div class="mx-auto w-full max-w-[550px]">
            <form action="index.php?modulo=agregar-tarea-apoyo&accion=agregar" method="POST" enctype="multipart/form-data">
                <div class="mb-5">
                    <label for="titulo" class="mb-3 block text-base font-medium text-[#07074D]">
                        Dirección
                    </label>
                    <input type="text" name="titulo" id="titulo" placeholder="Dirección del apoyo" class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" required />
                </div>
                <div class="mb-5">
